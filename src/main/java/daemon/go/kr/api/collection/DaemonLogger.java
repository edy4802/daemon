...
public class DaemonLogger {
    ...
    private DaemonLogger(){						// private 생성자 
        ...
        String logFilePath = (String)propsMap.get("logFilePath");
    	
        try {
        	logFile = new FileHandler(logFilePath + "log.txt", true); 		// 파일 핸들러 생성
        	warningFile = new FileHandler(logFilePath + "warning.txt", true); 	// 파일 핸들러 생성
        	fineFile = new FileHandler(logFilePath + "fine.txt", true);		// 파일 핸들러 생성
